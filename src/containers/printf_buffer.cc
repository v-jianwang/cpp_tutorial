...
inline void alloc_copy_and_format(const char *buf, int64_t length, int append_size,
		int64_t alloc_limit, const char *fmt, va_list ap, char **buf_out) {
...
//		// have written, not including the '\0'.
		...
		if (size < STATIC_DATA_SIZE - length_) {
			...
			char *new_ptr;
			alloc_copy_and_format(ptr_, length_, size, round_up_to_power_of_two(length_ + size + 1),
					format, aq, &new_ptr);
